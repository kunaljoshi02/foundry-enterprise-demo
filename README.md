...
| **Tools & toolboxes** | Shared `insurance-tools` toolbox (MCP endpoint) plus a typed function tool |
| **Memory** | `insurance-memory` store attached to a prompt agent via `memory_search_preview`, scoped per user |
| **Identity / permissions** | System-assigned managed identities, per-agent instance identities, `Foundry Agent Consumer` RBAC for the A2A hop |
| **Observability** | App Insights + Log Analytics behind an Azure Monitor Private Link Scope |
...
```
docs/
  foundry-enterprise-demo-plan.md   Full plan + as-built inventory + Appendices A–E
infra/
  apim/
...
toolbox/
  toolbox.yaml / toolbox-v2.yaml    insurance-tools toolbox definitions
```
...
| `underwriting-risk-summarizer` | Prompt | `gpt-4.1` (direct) | Structured risk brief; **carries the Memory tool** |
